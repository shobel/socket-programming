...
void createMessage(int trans_id, char* message, char* newmsg){
    snprintf(newmsg, MSGSIZE, "%d:%s", trans_id, message);
}
...
int main(int argc, char *argv[]){
    ...
    char content[1] = "";
    createMessage(trans_id, content, msg);
    if (send(sockfd, msg, MSGSIZE, 0) == -1){
        perror("error: client sending");
    }   
    printf("CLIENT: sent DISCOVERY message with transaction id %d\n", trans_id);
    ...
    //send REQUEST
    trans_id = generateTransactionId();
    createMessage(trans_id, ip, msg);
    if (send(sockfd, msg, MSGSIZE, 0) == -1){
        perror("error: client sending");
    }   
    printf("CLIENT: sent REQUEST for ip %s with transaction id %d\n", ip, trans_id);
